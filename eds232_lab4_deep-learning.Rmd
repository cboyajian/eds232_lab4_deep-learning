...
model_2sp <- keras_model_sequential() %>% 
  layer_flatten() %>%
...
```{r}
results_2sp <- model_2sp %>% evaluate(test_generator_2sp)
...
```{r}
# model using neural nets
model_10sp <- keras_model_sequential() %>% 
...
```{r}
results_10sp <- model_10sp %>% evaluate(test_generator_10sp)
